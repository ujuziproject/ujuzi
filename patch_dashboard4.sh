...
# Replace lines 212 to 251 with replacement.txt
sed -i -e '212,251c\' -e "$(cat replacement.txt | sed 's/$/\\/')" src/components/Dashboard.tsx
# Oops, sed replacement multiline can be tricky. Let's use python or perl.
python3 -c '
import sys
...
for i, line in enumerate(lines):
    if "{/* MY CURRICULA Section */}" in line:
        start = i
        break

...
# Add UI for Learning Style below track details
ui_target = """        )}
...
# Also need to add loadProfile to scope. Let's lift it outside useEffect.
# Actually `loadProfile` is inside useEffect. Let's fix that.
...
# We need to clean up the previous fetch_target replacement which failed or succeeded. Let's do a direct replacement for the new effect.
# Oh wait, I replaced fetch_target first, so effect_target is different now. Let's rewrite the script for effect replacement.
